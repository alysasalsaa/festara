...
import { Check, X, Clock, MapPin, Phone, Loader2 } from "lucide-react";
        ...
        <div className="flex gap-2">
          {FILTERS.map(f => (
            <button key={f.value} onClick={() => setFilter(f.value)}
              className={`text-xs font-semibold px-3 py-2 rounded-xl transition-colors ${filter === f.value ? "bg-[#1CABB4] text-white" : "bg-white text-[#4A7A6D] border border-[#D4EAC8]"}`}>
              {f.label}
            </button>
          ))}
        </div>
          ...
          {applications.map((app) => {
            ...
            return (
              <div key={app.id} className="bg-white/80 backdrop-blur rounded-2xl p-5 shadow-sm">
                ...
                <div className="grid md:grid-cols-3 gap-3 text-xs text-[#4A7A6D] mb-3">
                  <div className="flex items-center gap-1.5"><MapPin size={12} /> {app.location}</div>
                  <div className="flex items-center gap-1.5"><Phone size={12} /> {app.phone}</div>
                  <div className="flex items-center gap-1.5"><Clock size={12} /> {new Date(app.created_at).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })}</div>
                </div>

                <div className="flex items-center gap-2 mb-3">
                  <span className="text-xs bg-[#E8F8F9] text-[#1CABB4] font-semibold px-2.5 py-1 rounded-full">{categoryLabel}</span>
                </div>
